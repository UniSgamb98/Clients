...
public class ImpostazioniService {
    ...
    public List<Forno> getForni() {
        ...
        try (PreparedStatement statement = database.getConnection().prepareStatement("SELECT ID, TECNOLOGIA, ANNO, MARCA, MODELLO FROM FORNI ORDER BY TECNOLOGIA, MARCA, MODELLO");
             ResultSet resultSet = statement.executeQuery()) {
            List<Forno> forni = new ArrayList<>();
            while (resultSet.next()) {
                forni.add(new Forno(UUID.fromString(resultSet.getString("ID")), resultSet.getString("TECNOLOGIA"), resultSet.getString("ANNO"), resultSet.getString("MARCA"), resultSet.getString("MODELLO")));
            }
            return forni;
        } catch (Exception e) {
    ...
    public void saveForni(List<Forno> forni) {
        ...
        try (PreparedStatement delete = database.getConnection().prepareStatement("DELETE FROM FORNI");
            ...
            for (Forno forno : forni) {
                ...
                insert.setString(4, forno.marca());
                insert.setString(5, forno.modello());
                insert.addBatch();
            }
            insert.executeBatch();
        } catch (Exception e) {
